//Consider the student database of N students and their marks.
// Make use of a hash table implementation to quickly insert and lookup students' Rollno and marks.
// Implement collision handling techniques-  linear probing with chaining without replacement. 
//linear probing with chaining without replacement
#include <bits/stdc++.h>
using namespace std;

struct rec //this is a structure having four data members
{
    int roll;
    int chain;
    int marks;
...
public:
    hashing()
    {
        for (int i = 0; i < 10; i++)//this is a constructor that is initializing all the rollno and chain entries to -1
        {
            hashtable[i].roll = -1;
            hashtable[i].chain = -1;
        }
    ...
    void insert(int roll1,string name1,int marks1)//function for inserting a new entry
    {
        ...
        while (1)//infinite loop
        {
                ...
                for (int i = x; i <= x + 10; i++)//entering the value in next empty location and updating the chain of that hash value
                {
            ...
            else if((hashtable[x].roll) % 10 == y && hashtable[x].chain != -1){//if this position is occupied by an element of the same hash value and follow up chain exists
                    while(hashtable[x].chain!=-1){
                        x=hashtable[x].chain;
                    }
                    for(int i=x;i<x+10;i++){
                        if((hashtable[i % 10].roll == -1)){
                            ...
                            hashtable[i%10].name=name1;
                            hashtable[i%10].marks=marks1;
                            hashtable[x%10].chain=i%10;
                            u++;
                            break;                    
                        }
                    }
            }
            else//incase and element of different hashvalue resides at target location
            {
                if (u > 0)
                {
                    break;
                ...
                for (int i = x; i <= x + 10; i++)
                {
                    ...
                    if (hashtable[w % 10].roll == -1)//searching immediately next empty spot in the hashtable
                    {
                        ...
                        hashtable[w % 10].marks=marks1;
                        //chain will remain -1 by default using constructor
                        u++;
                        break;
                    }
